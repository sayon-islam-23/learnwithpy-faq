...
def blogs_comments(request, tagline):
    post = BlogPost.objects.filter(tagline=tagline).first()
    ...
    if request.method=="POST":
        ...
        #comment = Comment(user = user, blog=post)
        comment.save()
    return render(request, "blog_comments.html", {'post':post, 'comments':comments})

def Delete_Blog_Post(request, tagline):
    posts = BlogPost.objects.get(tagline=tagline)
    if request.method == "POST":
        posts.delete()
        return redirect('/')
